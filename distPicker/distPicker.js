...
class distPicker{
    constructor(area, el){
        if (!window.$ || !window.jQuery){
            throw new Error("distPicker需要jQuery支持!");
        }
        this._setArea();

        this.$province = $(el.province || '#province');
        this.$city = $(el.city || '#city');
        this.$county = $(el.county || '#county');
    ...
    init(){
        this._createProvince(mapProvince);
        this._createCity(mapProvince[0]["city"]);
        this._createCounty(mapProvince[0]["city"][0]["area"]);
    ...
    listen(){
        ...
        this.$province.on("change", function() {
            let indexProvince = $(this).find("option:checked").index();

            let arrCity = _that._getCity(indexProvince);
            let arrCounty = _that._getCountry(indexProvince, 0);

            _that._createCity(arrCity);
            _that._createCounty(arrCounty);

            _that.$city.off().on("change", function() {
                let indexCity = $(this).find("option:checked").index();
                let arrCounty = _that._getCountry(indexProvince, indexCity);
                _that._createCounty(arrCounty);
            });
        })
    }

    _createProvince(arrProvince){
        let strProvince = "";
        let lenProvince = arrProvince.length;
        for(let i = 0; i < lenProvince; ++i) {
            strProvince += '<option>' + arrProvince[i]["name"] + '</option>';
        }
        this.$province.html(strProvince);
    }
    _createCity(arrCity){
        let strCity = "";
        let lenCity = arrCity.length;
        for(let i = 0; i < lenCity; ++i) {
            strCity += '<option>' + arrCity[i]["name"] + '</option>';
        }
        this.$city.html(strCity);
    }
    _createCounty(arrCounty) {
        let strCounty = "";
        let lenCounty = arrCounty.length;
        for(let i = 0; i < lenCounty; ++i) {
            strCounty += '<option>' + arrCounty[i] + '</option>';
        }
        this.$county.html(strCounty);
    }

    _getCity(indexProvince){
        return this.citySource[indexProvince]["city"];
    }
    _getCountry(indexProvince, indexCity){
        return this.citySource[indexProvince]["city"][indexCity]["area"];
    }
    _setArea(){
        this.citySource = [
