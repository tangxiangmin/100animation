!(function (factory) {
    ...
    $.fn.distPicker = function (param) {

        return this.each(function (index) {
            var $self = $(this);

            // 获取元素
            var $province = $self.find("[data-role='province']"),
                $city = $self.find("[data-role='city']"),
                $county = $self.find("[data-role='county']");

            // 功能函数
            var get = {
                city: function(indexProvince){
                    return mapProvince[indexProvince]["city"];
                },
                county: function(indexProvince, indexCity){
                    return mapProvince[indexProvince]["city"][indexCity]["area"];
                }
            };
            ...
            var dist = {
                ...
                strCity: "",
                strCounty: "",
                createProvince: function (arrProvince) {
                    for(var i = 0, len = arrProvince.length; i < len; ++i) {
                        this.strProvince += '<option>' + arrProvince[i]["name"] + '</option>';
                    }

                },
                createCity: function(arrCity){
                    this.strCity = '';
                    for(var i = 0, len = arrCity.length; i < len; ++i) {
                        this.strCity += '<option>' + arrCity[i]["name"] + '</option>';
                    }
                },
                createCounty: function(arrCounty) {
                    this.strCounty = '';
                    for(var i = 0, len = arrCounty.length; i < len; ++i) {
                        this.strCounty += '<option>' + arrCounty[i] + '</option>';
                    }
                ...
                listen: function () {
                    ...
                    $province.on("change", function() {
                        indexProvince = $(this).find("option:checked").index();

                        var arrCity = get.city(indexProvince);
                        var arrCounty = get.county(indexProvince, 0);

                        dist.createCity(arrCity);
                        dist.createCounty(arrCounty);

                        $city.html(_self.strCity);

                        $city.trigger("change");
                    });

                    $city.on("change", function() {
                        indexCity = $(this).find("option:checked").index();
                        var arrCounty = get.county(indexProvince, indexCity);
                        dist.createCounty(arrCounty);

                        $county.html(_self.strCounty);
                    });
                },
                render: function () {
                    this.createProvince(mapProvince);
                    this.createCity(mapProvince[0]["city"]);
                    this.createCounty(mapProvince[0]["city"][0]["area"]);

                    $province.html(this.strProvince);
                    $city.html(this.strCity);
                    $county.html(this.strCounty);
                },
                init: function () {
                    // 多个地区选择容器以数组形式传递参数，
                    // 单个选择容器可以直接传入对象
